...
function errorMessage(reason: unknown) {
  return reason instanceof Error ? reason.message.slice(0, 400) : "Unknown runtime error";
}
    ...
    return NextResponse.json(
      {
        ...
        detail: "This public browser session has reached its live-run limit. Try again after the window resets.",
      },
      {
        status: 429,
  ...
  const stream = createUIMessageStream<AegisUIMessage>({
    execute: async ({ writer }) => {
      ...
      emit({
        ...
        nodeId: "run-start",
        label: scenario.name,
        summary: "Agentic and fixed-rule lanes started concurrently against live public sources.",
        status: "running",
      ...
      const completed = agenticResult.status === "fulfilled" && rulesResult.status === "fulfilled";
      emit({
        lane: "system",
        type: "run_completed",
        nodeId: "run-complete",
        label: completed ? "Parallel comparison complete" : "Run completed with errors",
        summary: completed
